...
const vec3 ROSEWATER = vec3(244.0, 219.0, 214.0) / 255.0; // #f4dbd6
const vec3 FLAMINGO = vec3(240.0, 198.0, 198.0) / 255.0; // #f0c6c6
const vec3 PINK = vec3(245.0, 189.0, 230.0) / 255.0; // #f5bde6
const vec3 MAUVE = vec3(198.0, 160.0, 246.0) / 255.0; // #c6a0f6
const vec3 RED = vec3(237.0, 135.0, 150.0) / 255.0; // #ed8796
const vec3 MAROON = vec3(238.0, 153.0, 160.0) / 255.0; // #ee99a0
const vec3 PEACH = vec3(245.0, 169.0, 127.0) / 255.0; // #f5a97f
const vec3 YELLOW = vec3(238.0, 212.0, 159.0) / 255.0; // #eed49f
const vec3 GREEN = vec3(166.0, 218.0, 149.0) / 255.0; // #a6da95
const vec3 TEAL = vec3(139.0, 213.0, 202.0) / 255.0; // #8bd5ca
const vec3 SKY = vec3(145.0, 215.0, 227.0) / 255.0; // #91d7e3
const vec3 SAPPHIRE = vec3(125.0, 196.0, 228.0) / 255.0; // #7dc4e4
const vec3 BLUE = vec3(138.0, 173.0, 244.0) / 255.0; // #8aadf4
const vec3 LAVENDER = vec3(183.0, 189.0, 248.0) / 255.0; // #b7bdf8

// Color of every particle. Choose any palette constant above, e.g. BLUE or PINK.
const vec3 PARTICLE_COLOR = BLUE;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    for (int i = 0; i < PARTICLE_COUNT; ++i) {
        ...
        particleColor += PARTICLE_COLOR * alpha;
        coverage += alpha;
    }
